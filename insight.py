...
def compute_insight(entries):
    ...
    #
    # ⚠ 이 자리를 '실패'로 쓰면 안 된다. 10번의 기록은 **답이 나오는 지점이 아니라
    #   보기 시작하는 지점**이다. 쏠림이 없다는 것도 그 자체로 하나의 정보다 —
    #   "어느 기운에도 크게 흔들리지 않는다"로 담백하게 뒤집어 말한다.
    # ⚠ 거짓 약속 금지: "기록이 쌓이면 선명해질 거예요"(단정)가 아니라
    #   "드러나기도 합니다"(가능성)다. 계속 기록해도 끝내 쏠림이 없을 수 있다.
    return {
        **base,
        "pattern": "none",
        "headline": "아직 한쪽으로 쏠린 결이 없어요.",
        "support": ("어느 기운에도 크게 흔들리지 않는다는 뜻이기도 해요. "
                    "기록이 쌓이면 결이 드러나기도 합니다."),
        "detail": {"r": round(r, 4)},
    }
